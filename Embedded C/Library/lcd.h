...
// To write data R/W must be low, port H (DDRH = 255) must be output
#define LCD_WRITE PORTK_PK1 = 0;DDRH = 0xFF;
// To read data R/W must be high, port H (DDRH = 0) hust be input
...
//{code there} executes the code within the curly brackets - below happens to be macros 
...
/// @brief Lcd_Init() initializes the lcd display (must be called first)
void lcd_Init (void);
char lcd_Busy (void);
